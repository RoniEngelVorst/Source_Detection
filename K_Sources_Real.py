...
def run_k_sources_all_methods_on_Random():
    real_graphs = [
        ...
        (r"real_graphs\facebook-wosn-links\out.facebook-wosn-links", "facebook_friendships", " ", 0.157),
        # need to check
    ...
    for (path, graph_name, sep_char, P_range) in real_graphs:  # (path, graph_name, sep_char, P_range)
        ...
        for k in [2, 3, 5]:
            ...
            Find_Top_K_Sources_Random(path, graph_name, sep_char, P_range, k, file_name)
...
def Find_Top_K_Sources_Random(path, graph_name, sep_char, P_range, k, file_name):
